...
const SubItemManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (stored) setSubItems(JSON.parse(stored));
  }, []);

  const handleAdd = () => {
    const trimmed = value.trim();
    if (trimmed && !subItems.includes(trimmed)) {
      const updated = [...subItems, trimmed];
      setSubItems(updated);
  ...
  const handleDelete = (item: string) => {
    const updated = subItems.filter((i) => i !== item);
    setSubItems(updated);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(updated));
  };
  ...
  return (
    <section className="max-w-2xl mx-auto bg-white shadow-xl rounded-2xl p-6 border border-gray-100">
      <header className="mb-6">
        ...
        <p className="text-sm text-gray-500">Organize deeper navigation under menu items.</p>
      </header>
      ...
      <div className="flex items-center space-x-3 mb-6">
        <input
          ...
          placeholder="e.g. Permissions, Logs"
